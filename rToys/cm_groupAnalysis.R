...
# domain_names <- c("linear","nLog","bLog","nERBs","nDLFs_fyr")

domain_names <- c("linear","bLog","nERBs","nDLFs_fyr")
polyorder <- 1
nBins <- 40
nObservations <- 16
...
for (name in domain_names) {
  ...
  print(name)
  assign(name, plot_cortical_distance(params, polyorder, nBins, nObservations, name))
}

final_plot <- plot_grid(linear, bLog, nERBs, nDLFs_fyr, labels = c("A", "B", "C", "D"), ncol = 2)
final_plot
# save plot
save_plot(str_c("all_compare_spacing",".png"), final_plot, device = "png", base_height = NULL,
          base_aspect_ratio = 1.1, base_width =  6.9291339,dpi = 450)

# CMF and PI
# All
params <- get_parameters(cm_dataset_2plot, "nERBs")
cal_cortical_magnification(params, polyorder, nBins, nObservations, "All")

# anterior
cm_dataset %>% filter(Analysis == 'pRF') %>% 
  filter(Location %in% c("anterior")) ->  cm_dataset_2plot_pos
params <- get_parameters(cm_dataset_2plot_pos, "nERBs")
cal_cortical_magnification(params, polyorder, nBins, nObservations, "Anterior")

# posterior
cm_dataset %>% filter(Analysis == 'pRF') %>% 
  filter(Location %in% c("posterior")) ->  cm_dataset_2plot_pos
params <- get_parameters(cm_dataset_2plot_pos, "nERBs")
cal_cortical_magnification(params, polyorder, nBins, nObservations, "Posterior")

####

for (name in domain_names) {
  params <- get_parameters(cm_dataset_2plot_pos, name)
  print(name)
  assign(name, plot_cortical_distance(params, polyorder, nBins, nObservations, name))
}

final_plot <- plot_grid(linear, nLog, nERBs, nDLFs_fyr, labels = c("A", "B", "C", "D"), ncol = 2)

final_plot

# save plot
save_plot(str_c("ant_compare_spacing",".png"), final_plot, device = "png", base_height = NULL,
          base_aspect_ratio = 1.1, base_width =  6.9291339,dpi = 450)
...
for (name in domain_names) {
  params <- get_parameters(cm_dataset_2plot_pos, name)
  print(name)
  assign(name, bin_cortical_distance(params, polyorder, nBins, nObservations, name))
}

cd_plot <-rbind(linear, nLog, nERBs, nDLFs_fyr)

ggplot(cd_plot, aes(x=x)) + 
  geom_point(aes(y = y), colour="black") +
  labs( x = "Frequency (kHz)", y = "Cortical Distance (mm)") +
  theme(legend.text=element_text(size=9)) +
  geom_line(aes(y = yf), colour="red") +
  facet_wrap(~mag)
...
final_plot <- plot_grid(linear, nLog, nERBs, nDLFs_fyr, labels = c("A", "B", "C", "D"),
                        ncol = 2,label_size=14,align="hv")

final_plot

# save plot
save_plot(str_c("pos_compare_spacing",".png"), final_plot, device = "png", base_height = NULL,
          base_aspect_ratio = 1.3, base_width =  6.9291339,dpi = 450,
          ncol = 2, # we're saving a grid plot of 2 columns
          nrow = 2 # and 2 rows
          )

ggplot(data = cm_dataset_2plot, mapping = aes(x = Frequency_nERB, y = CorticalDistance_norm, color = r2)) +
...
# 
# 
# domain_names <- c("nLog","nERBs","nDLFs","nDLFs_fyr")
# 
# polyorder <- 1
# nBins <- 40
# nObservations <- 8
...
# }
# 
# final_plot <- plot_grid(nLog, nERBs, nDLFs, nDLFs_fyr, labels = c("A", "B", "C", "D"), ncol = 2)
# 
# final_plot
# 
# # save plot
# save_plot(str_c("compare_spacing_dlfs",".png"), final_plot, device = "png", base_height = NULL,
#           base_aspect_ratio = 1.1, base_width =  6.9291339,dpi = 450)
# 
# # kHz
